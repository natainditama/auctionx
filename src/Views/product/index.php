            ...
            <tbody>
              <?php if (count($model['products']) > 0) : ?>
                <?php foreach ($model['products'] as $key => $row) : ?>
                  <?php
                  $date = DateTime::createFromFormat("Y-m-d", $row['tgl']);
                  ...
                  <tr>
                    ...
                    <td>
                      ...
                      <!-- deleteModal -->
                      <div class="modal fade" id="deleteModal<?= $row['id_barang'] ?>" tabindex="-1" role="dialog" data-bs-keyboard="false" aria-labelledby="deleteModalLabel<?= $row['id_barang'] ?>" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
                          <div class="modal-content">
                            <div class="modal-header">
                              <h4 class="modal-title" id="deleteModalLabel<?= $row['id_barang'] ?>">Delete <?= $row['nama_barang']; ?></h4>
                              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
                              </button>
                            </div>
                            <form action="./product/<?= $row['id_barang'] ?>/destroy" method="post">
                      ...
                      <button data-bs-toggle="modal" data-bs-target="#updateModal<?= $row['id_barang'] ?>" class="btn btn-ghost btn-icon btn-sm rounded-circle texttooltip" data-template="editTwo">
                      ...
                      <!-- UpdateModal -->
                      <div class="modal fade" id="updateModal<?= $row['id_barang'] ?>" tabindex="-1" role="dialog" data-bs-keyboard="false" aria-labelledby="updateLabel" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
                          <div class="modal-content">
                            <div class="modal-header">
                              <h4 class="modal-title" id="updateLabel">Create new product</h4>
                              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
                              </button>
                            </div>
                            <form action="./product/<?= $row['id_barang'] ?>/update" method="post">
                              <div class="modal-body">
                                <input type="hidden" name="id" value="<?= $row['id_barang'] ?>" required>
                                <div class="mb-3 col-12">
                                  <label class="form-label">Name <span class="text-danger">*</span></label>
                                  <input type="text" class="form-control" name="name" placeholder="Enter product name" required value="<?= $row['nama_barang'] ?>">
                                </div>
                                <div class="mb-3 col-12">
                                  <label class="form-label">Date</label>
                                  <input type="date" class="form-control" name="date" placeholder="Enter product date" required value="<?= $row['tgl'] ?>">
                                </div>
                                <div class="mb-3 col-12">
                                  <label class="form-label">Price <span class="text-danger">*</span></label>
                                  <input type="number" class="form-control" name="price" placeholder="Enter product price" required required value="<?= $row['harga_awal'] ?>">
                                </div>
                                <div class="col-12">
                                  <label class="form-label">Description <span class="text-danger">*</span></label>
                                  <textarea class="form-control" name="desc" cols="3" rows="5" placeholder="Enter product price" required><?= $row['deskripsi_barang'] ?></textarea>
                                </div>
                              </div>
                              <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
